...
export const generatePDFReport = (result: PredictionResult): void => {
  ...
  pdf.setFontSize(20)
  pdf.text('LINKLOCK', margin + 40, 20)
  ...
  pdf.setFont('helvetica', 'normal')
  pdf.setFontSize(7)
  pdf.text('LINKLOCK Professional Security Report', margin, footerY + 6)
  ...
  const disclaimerLine2 = 'LINKLOCK is not liable for decisions made based on this analysis.'
  ...
  pdf.text('© 2025 LINKLOCK. All rights reserved.', contactX, footerY + 6)

  // Save the PDF
  const fileName = `LINKLOCK_Professional_Report_${reportId}.pdf`
  pdf.save(fileName)
}
